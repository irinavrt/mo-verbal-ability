...
```{r fig.width = 14, fig.asp=.3, fig.cap="Figure 3. The same scatter plot as in Figure 1 for verbal ability effects calculated within each of seven ideology-based segments of the population."}
...
m_polv <- lm(estimate ~ polviews_full, wordsum_polv_eff)
...
wordsum_eff %>% 
  ...
  select(question, est, hfvl_advantage) %>% 
  arrange(desc(abs(hfvl_advantage))) %>% 
  kable(digits = 2, 
        format = "html",
        col.names = c("Question", "Verb.ab. effect", "UAA"),
